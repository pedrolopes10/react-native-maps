...
class MapView extends React.Component {
    ...
    setCamera(camera, options = {}) {
        if (this.map.current) {
            ...
            MapViewNativeComponent_1.Commands.setCamera(this.map.current, camera, edgePadding);
        }
    }
    animateCamera(camera, options = {}) {
        if (this.map.current) {
            const { edgePadding = { top: 0, right: 0, bottom: 0, left: 0 }, duration = 500, } = options;
            MapViewNativeComponent_1.Commands.animateCamera(this.map.current, camera, duration, edgePadding);
        }
    }
    animateToRegion(region, duration = 500, options = {}) {
        if (this.map.current) {
            const { edgePadding = { top: 0, right: 0, bottom: 0, left: 0 }, } = options;
            MapViewNativeComponent_1.Commands.animateToRegion(this.map.current, region, duration, edgePadding);
        }
    }
    fitToElements(options = {}) {
        if (this.map.current) {
            const { edgePadding = { top: 0, right: 0, bottom: 0, left: 0 }, animated = true, duration = 500, } = options;
            MapViewNativeComponent_1.Commands.fitToElements(this.map.current, edgePadding, animated, duration);
        }
    }
