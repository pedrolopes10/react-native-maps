...
const CameraShape = PropTypes.shape({
  ...
  centerCoordinateDistance: PropTypes.number.isRequired,
...
class MapView extends React.Component {
  ...
  setCamera(camera, opts) {
    this._runCommand('setCamera', [camera, (opts && opts.edgePadding) || null]);
  }

  animateCamera(camera, opts) {
    this._runCommand('animateCamera', [camera, (opts && opts.duration) || 500, (opts && opts.edgePadding) || null]);
  }
  ...
  animateToRegion(region, duration, options = {}) {
    const {
        edgePadding = { top: 0, right: 0, bottom: 0, left: 0 },
    } = options;
    this._runCommand('animateToRegion', [region, duration || 500, edgePadding]);
  }

  animateToCoordinate(latLng, duration, options = {}) {
      const {
          edgePadding = { top: 0, right: 0, bottom: 0, left: 0 },
      } = options;
      this._runCommand('animateToCoordinate', [latLng, duration || 500, edgePadding]);
  }
  ...
  fitToElements(animated, duration, options = {}) {
    const {
        edgePadding = { top: 0, right: 0, bottom: 0, left: 0 }
    } = options;
    this._runCommand('fitToElements', [animated, duration || 0, edgePadding]);
  }
